...
var CVSlider = function (el, _options) {
	...
	this.formatNumber = formatNumberCurry(options.numCharacters, options.maxPrecision, !this.isVertical);
...
CVSlider.eventHandlers = {
	input: {
		'change': function (event) {
			this.value = this.input.value; // rounds / clamps value and runs update if necessary
		},
...
function parseOptions(el, _options) {
	var containerElement, inputElement, elementAttributes, instanceOptions, options, spec;
	...
	if (el instanceof HTMLElement) {
		...
		elementAttributes = elementAttributes.reduce(function(obj, attr) {
			var floatValue = parseFloat(attr.value, 10);
			obj[attr.nodeName] = !isNaN(floatValue) ? floatValue : attr.value.toString();
			return obj; 
		}, {});
	...
	(_options instanceof Object && (spec = CVSlider.specs[_options.spec]));
...
function formatNumberCurry(_numCharacters, _maxPrecision, isFixedWidth) {	
